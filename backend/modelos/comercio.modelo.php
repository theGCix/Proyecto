...
class ModeloComercio{
	...
	static public function mdlActualizarScript($tabla, $id, $datos){
		...
		$apiFacebook = trim($datos["apiFacebook"]);
		$pixelFacebook = trim($datos["pixelFacebook"]);
		$googleAnalytics = trim($datos["googleAnalytics"]);
		...
		$stmt->bindParam(":apiFacebook", $apiFacebook, PDO::PARAM_STR);
		$stmt->bindParam(":pixelFacebook", $pixelFacebook, PDO::PARAM_STR);
		$stmt->bindParam(":googleAnalytics", $googleAnalytics, PDO::PARAM_STR);
		$stmt->bindParam(":id", $id, PDO::PARAM_INT);
